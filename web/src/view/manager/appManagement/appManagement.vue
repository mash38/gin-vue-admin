  ...
  <div>
      ...
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline"
        ...
        <template v-if="showAllQuery">
          ...
          <el-form-item label="应用名称" prop="app">
            <el-input v-model="searchInfo.app" placeholder="搜索条件" />
          </el-form-item>
    ...
    <div class="gva-table-box">
      ...
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="ID"
        ...
        <el-table-column align="left" label="应用名称" prop="app" width="120" />

        <el-table-column align="left" label="代号" prop="title" width="120" />
        <el-table-column align="left" label="客户ID" prop="customer_id" width="120" />

        <el-table-column label="logo" prop="logo" width="200">
          <template #default="scope">
            <el-image preview-teleported style="width: 100px; height: 100px" :src="getUrl(scope.row.logo)"
              fit="cover" />
          </template>
        </el-table-column>
        <el-table-column label="ico" prop="ico" width="200">
          <template #default="scope">
            <el-image preview-teleported style="width: 100px; height: 100px" :src="getUrl(scope.row.ico)" fit="cover" />
          </template>
        </el-table-column>
        <el-table-column align="left" label="版本号" prop="version" width="120" />

        <el-table-column label="安卓链接" prop="android" width="200">
          <template #default="scope">
            <ArrayCtrl v-model="scope.row.android" />
          </template>
        </el-table-column>
        <el-table-column label="苹果链接" prop="ios" width="200">
          <template #default="scope">
            <ArrayCtrl v-model="scope.row.ios" />
          </template>
        </el-table-column>
        <el-table-column label="应用链接" prop="nav" width="200">
          <template #default="scope">
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
          <template #default="scope">
            ...
            <el-button type="primary" link icon="edit" class="table-button"
              @click="updateAppManagementFunc(scope.row)">编辑</el-button>
    ...
    <el-drawer destroy-on-close :size="appStore.drawerSize" v-model="detailShow" :show-close="true"
      ...
      <el-descriptions :column="1" border>
        ...
        <el-descriptions-item label="logo">
          <el-image style="width: 50px; height: 50px" :preview-src-list="returnArrImg(detailForm.logo)"
            :src="getUrl(detailForm.logo)" fit="cover" />
        </el-descriptions-item>
        <el-descriptions-item label="ico">
          <el-image style="width: 50px; height: 50px" :preview-src-list="returnArrImg(detailForm.ico)"
            :src="getUrl(detailForm.ico)" fit="cover" />
        </el-descriptions-item>
        <el-descriptions-item label="版本号">
          {{ detailForm.version }}
        </el-descriptions-item>
        <el-descriptions-item label="安卓链接">
          <ArrayCtrl v-model="detailForm.android" />
        </el-descriptions-item>
        <el-descriptions-item label="苹果链接">
          <ArrayCtrl v-model="detailForm.ios" />
        </el-descriptions-item>
...
const getTableData = async () => {
  const table = await getAppManagementList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async () => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    const res = await deleteAppManagementByIds({ IDs })
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '删除成功'
...
// 更新行
const updateAppManagementFunc = async (row) => {
  const res = await findAppManagement({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// 删除行
const deleteAppManagementFunc = async (row) => {
  const res = await deleteAppManagement({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
