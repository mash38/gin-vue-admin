  ...
  <el-input v-model="searchInfo.code" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt"width="180">
            ...
            <el-table-column align="left" label="推荐人ID" prop="parent_id" width="120" />

            <el-table-column align="left" label="客户类型" prop="c_type" width="120" />

            <el-table-column align="left" label="客户编码" prop="code" width="120" />

            <el-table-column align="left" label="客户来源" prop="source" width="120" />

        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            <template #default="scope">
            <el-button  type="primary" link class="table-button" @click="getDetails(scope.row)"><el-icon style="margin-right: 5px"><InfoFilled /></el-icon>查看</el-button>
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateCustomerUserFunc(scope.row)">编辑</el-button>
...
const getTableData = async() => {
  const table = await getCustomerUserList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCustomerUserByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCustomerUserFunc = async(row) => {
    const res = await findCustomerUser({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCustomerUserFunc = async (row) => {
    const res = await deleteCustomerUser({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
